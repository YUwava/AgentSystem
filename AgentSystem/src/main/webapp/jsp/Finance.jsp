...
<script>
  ...
  */
    function test(){
        layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br>' + '<form action="${pageContext.request.contextPath}/figcontroller/typeinserty" method="post" id="Sub">类型名称:<input type="text" name="type" id="name">' +
    ...
    $(function () {
        $.ajax({
            url : '${pageContext.request.contextPath}/figcontroller/get_figlist',
            type : 'post',
            data : null,
            dataType : 'json',
            async: false,
    ...
    function updat(names,ids,finid){
        layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br>' + '<form action="${pageContext.request.contextPath}/figcontroller/updatFian" method="post" id="Sub">类型名称:<input type="text" name="type" id="name" value='+names+'>' +
    ...
    function delet(id){
        ...
        if (confirm(msg)==true){
            ...
            window.location.href="${pageContext.request.contextPath}/figcontroller/delete?id="+id;
            //你也可以在这里做其他的操作
        }else{
            return false;
        }
