...
<div class="layui-body">
    <table class="layui-table">
        ...
        <br/>
        <br/>
        <button type="button" class="layui-btn" onclick="test()">添加证件类型</button><br/><span style="color: #eb270f">${dlet}</span>
        <thead>
        <tr>
            <th>序号</th>
            <th>配置类型</th>
...
<script>
  ...
  */
    function test(){
        layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br>' + '<form action="${pageContext.request.contextPath}/Type_DocunCon/InsertyDocunCon" method="post" id="Sub">类型名称:<input type="text" name="type" id="name">' +
    ...
    $(function () {
        $.ajax({
            url : '${pageContext.request.contextPath}/Type_DocunCon/get_DocunCon',
            type : 'post',
            data : null,
            dataType : 'json',
            async: false,
    ...
    function updat(names,ids,finid){
        layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br>' + '<form action="${pageContext.request.contextPath}/Type_DocunCon/updatDocunCon" method="post" id="Sub">类型名称:<input type="text" name="type" id="name" value='+names+'>' +
    ...
    function delet(id){
        ...
        if (confirm(msg)==true){
            ...
            window.location.href="${pageContext.request.contextPath}/Type_DocunCon/delete?id="+id;
            //你也可以在这里做其他的操作
        }else{
            return false;
        }
